...
#include "main/drivers/inc/control.h"

#include "main/estimator/include/estimator.h" // Добавить 
#include "esp_err.h"
#include "esp_log.h"
...
static const char *TAG = "Ctabilizer task >>>";
static bool isInit;
...
//STATIC_MEM_TASK_ALLOC(stabilizerTask, STABILIZER_TASK_STACKSIZE); Возможно тут должна быть запись в память
...
void stabilizerInit(StateEstimatorType estimator)
{
  if(isInit)
    return;
  // тут нужна инициализация сенсоров и прочего
...
{
  setMotor_spd(MOTOR_M1, motorPwm->motors.m1);
  setMotor_spd(MOTOR_M2, motorPwm->motors.m2);
  setMotor_spd(MOTOR_M2, motorPwm->motors.m3);
  setMotor_spd(MOTOR_M2, motorPwm->motors.m4);
...
static void stabilizerTask(void* param)
{
  ...
  systemWaitStart();
  ESP_LOGI(TAG, "Wait for sensor calibration...");
  ...
  systemWaitStart();
  ESP_LOGI(TAG, "Starting stabilizer loop");
    ...
    if (healthShallWeRunTest()) {
      ...
      if (!rateSupervisorValidate(&rateSupervisorContext, xTaskGetTickCount())) {
        if (!rateWarningDisplayed) {
          ESP_LOGI(TAG, "WARNING: stabilizer loop rate is off (%lu)",rateSupervisorLatestCount(&rateSupervisorContext));
